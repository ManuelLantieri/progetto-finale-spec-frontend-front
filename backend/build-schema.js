...
if (!existsSync(typesFilePath)) {
  console.error(
    "⛔ Errore: Il file types.ts non esiste. Questo file è necessario per il funzionamento del server. Per favore, crea il file e riavvia il server."
  );
...
function generateSchemaFromTypes() {
  ...
  if (exportedTypes.length === 0) {
    console.error(
      "⛔ Errore: Non sono stati trovati tipi esportati in types.ts. Assicurati di avere almeno un tipo esportato e riavvia il server."
    );
    process.exit(1); // Termina il processo con un codice di errore
  }
    ...
    if (ts.isTypeLiteralNode(typeDecl.type)) {
      typeDecl.type.members.forEach((member) => {
        if (ts.isPropertySignature(member) && member.name) {
          ...
          if (isExported) {
            if (propName === "title") {
              ...
              schemaProperties.push(
                `  ${propName}: z.string({ required_error: "Title is required" })${
                  isOptional ? ".optional()" : ""
                },`
              );
              return;
              ...
              schemaProperties.push(
                `  ${propName}: z.string({ required_error: "Category is required" })${
                  isOptional ? ".optional()" : ""
                },`
              );
              return;
          ...
          let zodType = generateZodTypeForNode(
            member.type,
            sourceFile,
            propName,
            allTypes
...
${isExported ? "export " : ""}const ${schemaName} = z.object({
  ${
    ...
    if (isExported) {
      ...
      readonlyPropertiesList.push(
        `  "${typeName.toLowerCase()}": [${readonlyProperties
          .map((prop) => `"${prop}"`)
          .join(", ")}]`
      );
...
export const validators = {
  ${exportedValidators
    .map((name) => `"${name.replace("validate", "").toLowerCase()}": ${name}`)
    .join(",\n  ")}
};
...
function hasModifier(node, modifierKind) {
  return (
    node.modifiers && node.modifiers.some((mod) => mod.kind === modifierKind)
  );
}
...
function generateZodTypeForNode(typeNode, sourceFile, propName, allTypes) {
  ...
  if (typeNode.kind === ts.SyntaxKind.StringKeyword) {
    ...
    const elementType = generateZodTypeForNode(
    ...
    const formattedOptions =
      typeDescriptions.length > 1
        ? typeDescriptions.slice(0, -1).join(", ") +
          " or " +
          typeDescriptions[typeDescriptions.length - 1]
        : typeDescriptions[0];
    ...
    return `z.union([${typeDescriptions
      .map((desc) => `z.literal(${desc})`)
      .join(", ")}], { invalid_type_error: "${errorMsg}" })`;
    ...
    if (hasRestElement) {
      ...
      const restIndex = typeNode.elements.findIndex((e) =>
        ts.isRestTypeNode(e)
      );
      ...
      if (
        ts.isRestTypeNode(restElement) &&
        ts.isArrayTypeNode(restElement.type)
      ) {
        // If the rest element is an array type, get the element type directly
        restElementType = generateZodTypeForNode(
          restElement.type.elementType,
          sourceFile,
          propName,
          allTypes
        );
        ...
        if (restElementType.startsWith("z.array(")) {
          restElementType = restElementType.substring(
            8,
            restElementType.length - 1
          );
        }
      } else {
      ...
      const tupleTypes = typeNode.elements.map((e) =>
        generateZodTypeForNode(e, sourceFile, propName, allTypes)
      );
    ...
    if (allTypes.has(typeName)) {
      ...
      if (referencedType.type && ts.isUnionTypeNode(referencedType.type)) {
        const allStringLiterals = referencedType.type.types.every(
          (t) => ts.isLiteralTypeNode(t) && ts.isStringLiteral(t.literal)
        );
